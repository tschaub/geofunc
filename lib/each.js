  ...
  if (json.type === type) {
    ...
    switch (json.type) {
      ...
      case types.MultiPolygon: {
        if (type === types.LinearRing) {
          for (var i = 0, ii = json.coordinates.length; i < ii; ++i) {
            visitEachLinearRing(json.coordinates[i], visitor);
          }
        } else if (type === types.Coordinate) {
          for (var i = 0, ii = json.coordinates.length; i < ii; ++i) {
            var polygonCoordinates = json.coordinates[i];
            for (var j = 0, jj = polygonCoordinates.length; j < jj; ++j) {
              visitEachCoorinate(polygonCoordinates[j], visitor);
            }
          }
        }
        break;
      ...
      case types.MultiLineString: {
        if (type === types.Coordinate) {
          for (var i = 0, ii = json.coordinates.length; i < ii; ++i) {
            visitEachCoorinate(json.coordinates[i], visitor);
          }
        }
        break;
      }
      case types.Polygon: {
        if (type === types.LinearRing) {
          visitEachLinearRing(json.coordinates, visitor);
        } else if (type === types.Coordinate) {
          for (var i = 0, ii = json.coordinates.length; i < ii; ++i) {
            visitEachCoorinate(json.coordinates[i], visitor);
          }
      ...
      case types.LineString: {
        if (type === types.Coordinate) {
          visitEachCoorinate(json.coordinates, visitor);
        }
        break;
      }
      case types.Point: {
...
function visitEachCoorinate(coordinates, visitor) {
